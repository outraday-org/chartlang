...
- **Five rendering adapters.** Five full-surface example adapters
  (canvas2d, lightweight-charts, uPlot, ECharts, Konva) — each declares
  every plot kind + all 63 drawing kinds and runs conformance-green. They
  share one renderer-agnostic geometry layer (`decomposeDrawing` in
  `@invinite-org/chartlang-adapter-kit`), so no adapter owns a copy of the
  drawing math. Add one to your repo in a single command:
  ```bash
  npx @invinite-org/chartlang-cli add-adapter <library>
  ```
  Compare them — license, render tech, bundle size, install command — in
  the [adapter gallery](./docs/adapters/gallery.md).
