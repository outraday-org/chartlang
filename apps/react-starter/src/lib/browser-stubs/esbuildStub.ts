...
//
// Ported from apps/site/src/lib/browser-stubs/esbuildStub.ts (the
// compiler-coexistence invariant is shared between the two apps).
//
...
// `/api/compile` server route (which loads esbuild directly in Node). The
// browser-side language service is constructed here only for the parts that
// do NOT need compilation: hover, completions, signature help, definition.
 ...
 * server-side inside the `/api/compile` route.
 */
