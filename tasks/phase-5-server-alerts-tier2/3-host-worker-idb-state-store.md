...
- [ ] `idbStateStore({ dbName?, key, capBytes? })` exported from
      `@invinite-org/chartlang-host-worker/idb`.
- [ ] IDB object store creation, key isolation, round-trip, eviction,
      concurrent writes all tested against `fake-indexeddb`.
- [ ] Bench pair lands under `THRESHOLD_MS`.
- [ ] Subpath export declared in `package.json`; the import works
      from a consumer test (verify via the conformance suite Task 5
      smoke).
- [ ] README updated, ≤ 100 lines.
- [ ] 100% coverage maintained; `pnpm docs:check` green.
- [ ] Changeset committed.
