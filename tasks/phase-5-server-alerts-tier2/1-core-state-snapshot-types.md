...
- [ ] Three new types exported from `@invinite-org/chartlang-core` with
      JSDoc carrying `@since 0.5`, `@example`, `@experimental`.
- [ ] `expectType<JsonValue>(snapshotLiteral)` compiles — proves
      JsonValue-cleanliness at the type level.
- [ ] `@ts-expect-error` assertions cover the `snapshotVersion: 0`,
      `apiVersion: 2`, and unknown-field cases.
- [ ] `CORE_AMBIENT_SHIM` in the compiler mirrors the three new types
