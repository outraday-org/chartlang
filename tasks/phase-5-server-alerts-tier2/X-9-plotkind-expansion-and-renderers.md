...
- [x] `PlotKind` is 17 entries; the matching union/discriminant
      types align across core / adapter-kit / runtime.
- [x] `capabilities.plots(...)` accepts all 17 kinds; `PHASE_5_PLOT_KINDS`
      exported.
- [x] Runtime emit dispatch + canvas2d render cover each new kind.
- [x] 8 happy-path + 8 capability-gated conformance scenarios green.
- [x] Silent no-op semantics preserved (Phase-1 invariant from
