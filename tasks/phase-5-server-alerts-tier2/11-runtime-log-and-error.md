...
- [ ] `runtime.log.{info,warn,error}` + `runtime.error` exported and
      wired.
- [ ] `Capabilities.logs: false` → silent no-op (no diagnostic).
- [ ] 1000-log cap fires with `runtime-log-budget-exceeded`.
- [ ] `runtime.error()` halts compute, drops bar emissions, emits
      `runtime-error-thrown`, next bar runs.
- [ ] `STATEFUL_PRIMITIVES.size === 166`.
- [ ] All 4 conformance scenarios green.
