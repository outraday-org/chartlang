...
- [ ] `DrawingKind` count is 62; `DRAWING_KINDS.length === 62`.
- [ ] `STATEFUL_PRIMITIVES.size === 167`.
- [ ] `draw.table(opts)` compiles + runs; handle.update works.
- [ ] Viewport-anchored rendering verified per `position` value.
- [ ] Capability gating fires `unsupported-drawing-kind`.
