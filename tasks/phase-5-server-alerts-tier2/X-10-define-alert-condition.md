...
- [x] `defineAlertCondition` exported from core with full JSDoc.
- [x] Compiler extracts `manifest.alertConditions` from object-literal
      conditions; non-literal fails with `alert-condition-not-literal`.
- [x] `STATEFUL_PRIMITIVES.size === 164` after this task.
- [x] Runtime `signal(conditionId, fired)` emits per documented
      semantics; unknown / disabled cases emit the right diagnostics.
- [x] `RunnerEmissions.alertConditions` populated; validator covers
      shape.
- [x] All 3 conformance scenarios green.
- [x] canvas2d-adapter renders side panel; integration test pins ops.
