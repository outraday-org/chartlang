...
cursors, and axes — the smooth, TradingView-grade output the canvas2d /
uplot / konva adapters cannot match (see the "wobbly MA / thin candle"
investigation: the real win there was the 120-bar window; the WebGL
adapter additionally brings GPU AA + thick miter-joined line quads).
...
- Five adapters: `canvas2d` (reference, the only coverage-gated example),
  `lightweight-charts`, `uplot`, `echarts`, `konva`.
...
- Reuses the shared `ViewController` (pan/zoom + `initialVisibleBars`
  ...
  `mockCandleSource`, and `createWorkerHost` — never forks them.
...
visible-window math, or the candle-state override semantics — they are
shared. Port from invinite ONLY the GPU layers (context, programs,
shaders, buffers, ortho2d/viewport, Renderer, ChartController).
