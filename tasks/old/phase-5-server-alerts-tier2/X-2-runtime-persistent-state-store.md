...
- [x] `PersistentStateStore` sub-interface + `inMemoryPersistentStateStore`
      ship with full JSDoc.
- [x] `validateSnapshot` rejects every documented failure mode.
- [x] `createScriptRunner` accepts `persistentStateStore` +
      `persistenceIntervalMs`, exposes `warmStart()`, and flushes on
      `dispose()`.
- [x] Determinism test pins byte-identical emissions cold vs warm
      across a representative `ta.*` + `state.*` mix (≥25
      `fast-check` runs).
- [x] 100% coverage on every touched file; new lines exercised by
      tests.
- [x] Phase-4 runtime invariants (per `packages/runtime/CLAUDE.md`)
