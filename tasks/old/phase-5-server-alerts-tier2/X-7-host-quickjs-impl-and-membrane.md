...
- [x] `createQuickJsHost` boots a real `QuickJSAsyncContext` with
      memory + CPU caps configured.
- [x] All 5 cross-host parity tests pass — `host-quickjs` and
      `host-worker` emissions are byte-identical.
- [x] `quickjs-oom` and `step-overshoot` surface via `onHostError`
      as documented.
- [x] `loadError` / `fatal` paths cover script syntax errors +
      runtime throws.
- [x] Dispatcher bundle is self-contained; no host bindings exposed
      inside QuickJS.
- [x] CLAUDE.md documents the invariants; future contributors can't
      regress them silently.
- [x] 100% coverage; bench under threshold.
