...
- [x] Both files ship the 4-line provenance header.
- [x] Unit + property + golden + bench all green; bench under 5 ms.
- [x] Cache invariants tested (length-change → miss; identity-change
      → miss).
- [x] No look-ahead invariant pinned by property test.
- [x] 100% coverage on touched files.
- [x] No public export from the package barrel — the kernel stays
      internal.
