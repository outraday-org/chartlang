...
> **✓ Dependency satisfied (verified 2026-06-21).**
> `tasks/multi-library-adapters/` is implemented and merged (commits
> `03f59bf` "tasks 1-13" + `64cbe90`). Verified present in the workspace,
> so Tasks 5 and 7 can run:
> - All five example adapters
>   (`examples/{canvas2d,echarts,konva,lightweight-charts,uplot}-adapter`,
>   packages `chartlang-example-*-adapter`); the echarts adapter exports
>   `createEChartsAdapter` + `EChartsAdapterHandle` (and `runEChartsLoop`,
>   `ECHARTS_CAPABILITIES`).
> - The `@invinite-org/chartlang-adapter-kit` geometry layer (v1.3.0):
>   `decomposeDrawing`, `paintPrimitive`, and the `AlertEmission` /
>   `CandleEvent` types.
> - The CLI `chartlang add-adapter <id> [dir]` command
>   (`packages/cli/src/commands/addAdapter.ts`).
> - The offline bundles `BUNDLED_ADAPTERS`
>   (`ReadonlyArray<{ id, files }>`) + `ADAPTER_REGISTRY` under
>   `packages/cli/src/generated/adapters/**`, generated by
>   `pnpm adapters:generate` (`adapters:gate` byte-diffs them in CI).
>
> The earlier "0/15 — only canvas2d exists" blocker is **stale and has
> been removed**. The exact bundle/registry/command shapes Tasks 5 and 7
> rely on were re-verified against the real output (see those tasks for
> the corrected symbol names — e.g. array `.find(b => b.id === id)` not
> `BUNDLED_ADAPTERS[id]`, registry field `libraryRange`).
...
- **`apps/site/`** (`chartlang-site`, private, TanStack Start + shadcn
  ...
  - `ChartPane.tsx` — `createCanvas2dAdapter({ canvas, candleSource, … })`,
    `await adapter.host.load({ moduleSource, manifest })`, `runRendererLoop`,
    a **local** push-candle source (`createPushCandleSource`, inline) + the
    MTF `createResamplingCandlePump` path (local, in `secondaryStreams.ts`).
...
- **`packages/cli/`** — `chartlang add-adapter <id> [dir]` (dir defaults
  to `./<id>-adapter`) writes a complete, published-version-pinned adapter
  from `BUNDLED_ADAPTERS` (`packages/cli/src/generated/adapters/**`),
  offline. `BUNDLED_ADAPTERS` is a `ReadonlyArray<{ id, files }>`; each
  bundle's `package.json` already has `workspace:^` rewritten to `^<published>`
  and its `name` set to the `__PKG_NAME__` placeholder.
...
| Local push-candle source (`createPushCandleSource`, defined inline in the site `ChartPane`) + render driving | `apps/site/src/components/demo/ChartPane.tsx` (the push source is **local** to that file; only `createCanvas2dAdapter` + `runRendererLoop` come from `chartlang-example-canvas2d-adapter`) | Task 5 (**extract into a local `streamPump.ts`** — must NOT import the concrete-adapter `createCanvas2dAdapter`/`runRendererLoop`, to keep the seam clean) |
| `createResamplingCandlePump` (MTF resample) | `apps/site/src/components/demo/secondaryStreams.ts` | Task 5 (port — the site already keeps this local, so the seam is clean). NB: the symbol is `createResamplingCandlePump`, **not** `buildSecondaryStreams`; and it is distinct from the canvas2d adapter's own `createMultiStreamCandlePump` (which the site `ChartPane` does **not** use). |
