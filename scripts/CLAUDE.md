...
| `generate-skills-reference.ts` | `pnpm skills:generate` / `pnpm skills:gate` | Walks `ta.*`/`draw.*`/plot-family JSDoc plus the consolidated namespaces named in `NAMESPACE_TITLES` (`math` / `str` / `array` / `barstate`) → `skills/chartlang-coding/references/primitives.md`; `--check` byte-diffs against the committed file. Deep-imports `parsePrimitiveSource` / `parseDrawingSource` (source, not `dist/`) and mirrors their skip lists. The `## plot family` section is driven by `collectPlotFamily`, which reads the four core holes (`plot`/`hline`/`bgcolor`/`barcolor`) from `packages/core/src/plot/plot.ts` directly via the TS AST — those holes are author signatures with no `@formula`/`@warmup`, so they can't go through `parsePrimitiveSource`. The `## <ns>.*` sections are driven by `collectNamespace`, which reuses `parsePhase4DocEntry` + the shared `PHASE4_DOC_ENTRIES` registry (deep-imported from `packages/cli/src/commands/genPhase4Docs.ts`, source not `dist/`) so the source/symbol paths can't drift from `docs:gate`; each renders as ONE consolidated block (the whole `Object.freeze({...})` member list), not per-member. **`NAMESPACE_TITLES` is the single edit point for a fifth** — `collectNamespace`'s parameter type is derived from it and `renderReference` takes the collected docs as one ordered array, so adding a name there (plus its `PHASE4_DOC_ENTRIES` entry, its `docs/.vitepress/config.ts` sidebar row, and an `EXAMPLE_CATALOGUE` `primitives` credit for the new page) is the whole change; the array order IS the render order. There are **two** namespace collectors and which one applies is decided by the doc-page shape, not by taste: `collectNamespace(title)` looks up ONE entry by exact `title` (a namespace with a single consolidated page — `math` / `str` / `array` / `barstate`), while `collectMemberNamespace(prefix)` selects every entry whose `title` starts `"<prefix>."` and renders one `### <prefix>.<member>` block per page (`order`, whose four members each own a `docs/primitives/order/<member>.md`). Do not give a per-member namespace a synthetic consolidated entry to reach `collectNamespace` — the extra `docs/primitives/<ns>.md` page mints a bare `<ns>` id in `examples-coverage.ts`'s target set that no example can credit. `renderPlotBlock` and the per-member renderer share `renderSignatureBlock`. The parsed `sourceUrl` is intentionally NOT rendered (the reference carries no GitHub links). Never hand-edit `primitives.md` — re-run `pnpm skills:generate`. |
