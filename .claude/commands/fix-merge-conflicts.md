...
2. **Honor explicit per-file overrides from the user.** If the user's arguments
   ...
   below. Never generalize one override to sibling files (e.g. a `.tsx`
   override does **not** extend to its neighboring `CLAUDE.md`).
...
- **Order matters.** When both sides added entries to an ordered list (array,
  enum, switch cases, index file), include all entries and pick a sensible
  order.
...
2. **Reconcile against the actual code that is being kept.** If the file
   ...
   are additive context (`Discord managedReactiveScroller reference`,
   `Safari fallback note`, `BUFFER_CAP bounding rationale`) that still apply.
...
4. **Weave additive content in** rather than appending a "from main:" block.
   A merged CLAUDE.md should read like one voice, not a stitched-together
   diff.
5. **Never delete a whole new section** that only one side added
   (e.g. PR-side "Debug Logging" section, main-side "Safari fallback" bullet)
   just because the surrounding paragraph was conflicted. New sections are
   almost always pure additions.
6. **When two sides give different values for the same named constant or
   behavior** (e.g. `WINDOW_SIZE = 100` vs `150`, `TAIL_SIZE = 25` vs `50`),
   pick the value that matches the code being kept and update the prose
   around it — do **not** keep both values, that would be incorrect docs.
7. **User override applies here too.** If the user explicitly said "take
   `X.tsx` from PR 343 entirely, overwrite main's version", that override is
   about `X.tsx` only. The adjacent `CLAUDE.md` still gets a full doc merge
   unless the user also named it explicitly.
8. **When in doubt, keep more, not less.** A doc with a redundant sentence
...
- Did one side **re-export** something from an index or barrel file? Make sure
  the other side's new modules are also re-exported if appropriate.
...
- **An override for one file does not extend to its neighbors.** Merging
  `X.tsx` from one side does not license blanket-taking the sibling
  `X.CLAUDE.md` or any other nearby file — those still go through the full
  merge path (see the Documentation Files section).
