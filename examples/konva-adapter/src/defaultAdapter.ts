...
/**
 * Headless `Adapter` exposing {@link KONVA_CAPABILITIES} as its
 * `id` / `name` / `capabilities` triple. The multi-adapter
 * `scripts/run-conformance.ts` wrapper (Task 13) auto-imports this as
 * the default export of `chartlang-example-konva-adapter` and feeds it
 * to `runConformanceSuite`, which reads `capabilities` only — the
 * `candles`, `onEmissions`, `dispose` methods are no-ops so the
 * conformance harness does not spin up a Worker host or a real Konva
 * `Stage` for what is fundamentally an emission-contract test.
 *
 * Production callers should use `createKonvaAdapter` instead, which
 * wires a real Konva scene-graph + worker host against an injected
 * Konva namespace.
 *
 * @since 1.4
 * @stable
 * @example
 *     import defaultAdapter, { DEFAULT_ADAPTER } from "chartlang-example-konva-adapter";
...
export const DEFAULT_ADAPTER: Adapter = Object.freeze({
    id: "konva-example-default",
    name: "Konva Example Adapter (default)",
