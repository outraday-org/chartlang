...
import { UPLOT_CAPABILITIES } from "./capabilities.js";
import { DEFAULT_ADAPTER } from "./defaultAdapter.js";

describe("DEFAULT_ADAPTER", () => {
    it("exposes the uPlot capability bag verbatim", () => {
        expect(DEFAULT_ADAPTER.capabilities).toBe(UPLOT_CAPABILITIES);
    });

    it("declares a stable id + name pair", () => {
        expect(DEFAULT_ADAPTER.id).toBe("uplot-example-default");
        expect(DEFAULT_ADAPTER.name).toBe("uPlot Example Adapter (default)");
    });
