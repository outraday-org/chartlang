...
function emission(state: ElliottTripleComboState): DrawingEmission {
    return {
        kind: "drawing",
        handleId: "h",
        drawingKind: "elliott-triple-combo",
        op: "create",
        state,
        bar: 0,
        time: 0,
...
describe("renderElliottTripleCombo", () => {
    it("strokes one open polyline (1 beginPath / 1 moveTo / 6 lineTo / 1 stroke)", () => {
        const ctx = new MockCanvas2DContext();
        renderElliottTripleCombo(ctx, emission(STATE), VIEW);
    ...
    it("labels every pivot with the default S/W/X1/Y/X2/Zi/Z sequence", () => {
        const ctx = new MockCanvas2DContext();
        renderElliottTripleCombo(ctx, emission(STATE), VIEW);
        const texts = ctx.calls
            .filter((c) => c.kind === "fillText")
            .map((c) => (c.kind === "fillText" ? c.text : ""));
        expect(texts).toEqual(["S", "W", "X1", "Y", "X2", "Zi", "Z"]);
    });
    ...
    it("honours state.labels override when length matches", () => {
        ...
        const texts = ctx.calls
            .filter((c) => c.kind === "fillText")
            .map((c) => (c.kind === "fillText" ? c.text : ""));
        expect(texts).toEqual(["α", "β", "γ", "δ", "ε", "ζ", "η"]);
    });
