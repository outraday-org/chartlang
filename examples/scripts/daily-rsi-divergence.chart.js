...
// packages/compiler/examples/scripts/daily-rsi-divergence.chart.ts
var daily_rsi_divergence_chart_default = defineIndicator({
  ...
  compute({ bar, ta: ta3, plot: plot3, state: state3, timeframe: timeframe2, inputs }) {
    if (!timeframe2.isdaily)
      return;
    const rsi = ta3.rsi("examples/scripts/daily-rsi-divergence.chart.ts:19:21#0", bar.close, inputs.length);
    const barsSince = state3.int("examples/scripts/daily-rsi-divergence.chart.ts:20:27#0", 0);
    const overbought = rsi.current > 70;
    const oversold = rsi.current < 30;
    barsSince.value = overbought || oversold ? 0 : barsSince.value + 1;
    plot3("examples/scripts/daily-rsi-divergence.chart.ts:24:9#0", rsi.current, { color: "#7c3aed", title: "RSI", pane: "rsi" });
    plot3("examples/scripts/daily-rsi-divergence.chart.ts:25:9#0", barsSince.value, { color: "#94a3b8", title: "Bars since divergence", pane: "rsi" });
  },
  outputs: [{ title: "RSI", kind: "series-number" }, { title: "Bars since divergence", kind: "series-number" }]
});
export {
  daily_rsi_divergence_chart_default as default
};
export const __manifest = {"apiVersion":1,"kind":"indicator","name":"Daily RSI Divergence","inputs":{"tf":{"kind":"interval","defaultValue":"1D","title":"Main timeframe"},"length":{"kind":"int","defaultValue":14,"min":2,"max":200,"title":"RSI length"}},"capabilities":["indicators"],"requestedIntervals":[],"userPickableInterval":true,"seriesCapacities":{},"maxLookback":0,"outputs":[{"title":"RSI","kind":"series-number"},{"title":"Bars since divergence","kind":"series-number"}],"plots":[{"slotId":"examples/scripts/daily-rsi-divergence.chart.ts:24:9#0","kind":"line","title":"RSI"},{"slotId":"examples/scripts/daily-rsi-divergence.chart.ts:25:9#0","kind":"line","title":"Bars since divergence"}]};
daily_rsi_divergence_chart_default = Object.freeze({ ...daily_rsi_divergence_chart_default, manifest: {"apiVersion":1,"kind":"indicator","name":"Daily RSI Divergence","inputs":{"tf":{"kind":"interval","defaultValue":"1D","title":"Main timeframe"},"length":{"kind":"int","defaultValue":14,"min":2,"max":200,"title":"RSI length"}},"capabilities":["indicators"],"requestedIntervals":[],"userPickableInterval":true,"seriesCapacities":{},"maxLookback":0,"outputs":[{"title":"RSI","kind":"series-number"},{"title":"Bars since divergence","kind":"series-number"}],"plots":[{"slotId":"examples/scripts/daily-rsi-divergence.chart.ts:24:9#0","kind":"line","title":"RSI"},{"slotId":"examples/scripts/daily-rsi-divergence.chart.ts:25:9#0","kind":"line","title":"Bars since divergence"}]} });
