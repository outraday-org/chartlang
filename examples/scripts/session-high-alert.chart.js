...
// packages/compiler/examples/scripts/session-high-alert.chart.ts
var session_high_alert_chart_default = defineIndicator({
  ...
  compute({ bar, state: state3, alert: alert3, plot: plot3, ta: ta3, barstate: barstate2, inputs }) {
    const high = state3.float("examples/scripts/session-high-alert.chart.ts:18:22#0", Number.NaN);
    const isSessionOpen = barstate2.isfirst || bar.time % 864e5 === 0;
    if (isSessionOpen) {
      high.value = bar.high;
    } else if (Number.isNaN(high.value) || bar.high > high.value) {
      high.value = bar.high;
    }
    plot3("examples/scripts/session-high-alert.chart.ts:25:9#0", high.value, { color: "#ff9900", title: "Session high" });
    if (inputs.alertOnCross && ta3.crossover("examples/scripts/session-high-alert.chart.ts:26:36#0", bar.close, high.value).current) {
      alert3("examples/scripts/session-high-alert.chart.ts:27:13#0", "Close crossed session high", { severity: "info" });
    }
  },
  outputs: [{ title: "Session high", kind: "series-number" }]
});
export {
  session_high_alert_chart_default as default
};
export const __manifest = {"apiVersion":1,"kind":"indicator","name":"Session High Alert","inputs":{"alertOnCross":{"kind":"bool","defaultValue":true,"title":"Alert on cross"}},"capabilities":["alerts","indicators"],"requestedIntervals":[],"userPickableInterval":false,"seriesCapacities":{},"maxLookback":0,"overlay":true,"outputs":[{"title":"Session high","kind":"series-number"}],"plots":[{"slotId":"examples/scripts/session-high-alert.chart.ts:25:9#0","kind":"line","title":"Session high"}]};
session_high_alert_chart_default = Object.freeze({ ...session_high_alert_chart_default, manifest: {"apiVersion":1,"kind":"indicator","name":"Session High Alert","inputs":{"alertOnCross":{"kind":"bool","defaultValue":true,"title":"Alert on cross"}},"capabilities":["alerts","indicators"],"requestedIntervals":[],"userPickableInterval":false,"seriesCapacities":{},"maxLookback":0,"overlay":true,"outputs":[{"title":"Session high","kind":"series-number"}],"plots":[{"slotId":"examples/scripts/session-high-alert.chart.ts:25:9#0","kind":"line","title":"Session high"}]} });
