...
- 8fb0338: Add the compile-time `unsupported-input-kind` gate behind a new `declaredInputKinds` option
...
- 3932418: Derive the `"orders"` capability from `order.*` callsites
  ...
  `"orders"` is the one capability id no script _kind_ seeds; it exists only if a
