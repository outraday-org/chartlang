...
- 3932418: `order.*`: the script-facing market-intent namespace (RFC 0002)

  A chartlang script could describe a signal but never _state_ one. The only way
  ...
  - `order.position()` reads the _nominal_ position — `{ size, avgPrice,
