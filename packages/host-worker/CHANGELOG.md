...
- 917caf6: Rebuild `worker-boot` so it inlines the gated runtime.
...
- 65491dc: Collapse `filterEmissions`'s five hand-copied validate loops into one helper
...
- 3932418: Carry and validate the `orders` channel across the worker boundary
...
- bc93986: `ScriptHost` gains `exportSnapshot` / `importSnapshot`, plus an opt-in IDB boot store
