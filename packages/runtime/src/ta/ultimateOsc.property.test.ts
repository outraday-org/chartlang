...
describe("ta.ultimateOsc — property invariants", () => {
    ...
    it("determinism: same input → identical output", () => {
        fc.assert(
            fc.property(fc.array(arbBar, { minLength: 30, maxLength: 60 }), (bars) => {
                const a = harness(bars, bars.length + 1, () => ultimateOsc("slot").current);
                const b = harness(bars, bars.length + 1, () => ultimateOsc("slot").current);
