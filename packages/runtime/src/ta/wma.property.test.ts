...
describe("ta.wma — property invariants", () => {
        ...
        fc.assert(
            fc.property(fc.array(arbBar, { minLength: 5, maxLength: 60 }), (bars) => {
                const out = harness(
                    bars,
                    bars.length + 1,
                    (bar) => wma("slot", bar.close, 4).current,
                );
            ...
            fc.property(
                ...
                (bars, length) => {
                    const out = harness(
                        bars,
                        bars.length + 1,
                        (bar) => wma("slot", bar.close, length).current,
                    );
    ...
    it("incremental output equals wmaFloat64 reference within 1e-8", () => {
        fc.assert(
            fc.property(fc.array(arbBar, { minLength: 20, maxLength: 80 }), (bars) => {
                const closes = new Float64Array(bars.map((b) => b.close));
                const expected = wmaFloat64(closes, 5);
                const actual = harness(
                    bars,
                    bars.length + 1,
                    (bar) => wma("slot", bar.close, 5).current,
                );
