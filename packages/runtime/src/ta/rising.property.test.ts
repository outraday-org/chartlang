...
// Brute-force oracle over the closed close series: rising[t] iff each of the
// trailing `length` deltas is strictly positive and every windowed value is
// finite; false during warmup (t < length).
function referenceRising(closes: ReadonlyArray<number>, length: number): boolean[] {
...
describe("ta.rising — property invariants", () => {
            ...
            fc.property(
                ...
                (bars, length) => {
                    const out = harness(
                        bars,
                        bars.length + 1,
                        (bar) => rising("slot", bar.close, length).current,
                    );
    ...
    it("rising and falling are never both true on the same bar", () => {
        fc.assert(
            fc.property(
                fc.array(arbBar, { minLength: 10, maxLength: 50 }),
                fc.integer({ min: 1, max: 6 }),
        ...
        fc.assert(
            fc.property(
                ...
                (bars, length) => {
                    ...
                    const expected = referenceRising(
