...
describe("ta.highest — property invariants", () => {
        ...
        fc.assert(
            fc.property(fc.array(arbBar, { minLength: 5, maxLength: 60 }), (bars) => {
                const out = harness(
                    bars,
                    bars.length + 1,
                    (bar) => highest("slot", bar.high, 4).current,
                );
            ...
            fc.property(
                ...
                (bars, length) => {
                    const out = harness(
                        bars,
                        bars.length + 1,
                        (bar) => highest("slot", bar.high, length).current,
                    );
    ...
    it("incremental output equals brute-force rolling max within 1e-12", () => {
        fc.assert(
            fc.property(fc.array(arbBar, { minLength: 20, maxLength: 80 }), (bars) => {
                ...
                for (let i = length - 1; i < bars.length; i += 1) {
                    ...
                    for (let j = i - length + 1; j <= i; j += 1) {
                        expected = Math.max(expected, bars[j].high);
                    }
    ...
    it("output is always greater than or equal to current source past warmup", () => {
        fc.assert(
            fc.property(fc.array(arbBar, { minLength: 10, maxLength: 50 }), (bars) => {
                ...
                for (let i = length - 1; i < bars.length; i += 1) {
                    expect(out[i]).toBeGreaterThanOrEqual(bars[i].high);
                }
            }),
            { numRuns: 30 },
        );
