...
describe("ta.nvi — property invariants", () => {
    ...
    it("matches the brute-force recurrence (lower-volume-only updates)", () => {
        fc.assert(
            fc.property(
    ...
    it("determinism: same input → identical output", () => {
        fc.assert(
            fc.property(
                ...
                (n, seed) => {
                    const bars = syntheticBars(n, seed);
                    const a = harness(bars, bars.length + 1, () => nvi("slot").current);
                    const b = harness(bars, bars.length + 1, () => nvi("slot").current);
