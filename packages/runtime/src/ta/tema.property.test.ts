...
describe("ta.tema — property invariants", () => {
        ...
        fc.assert(
            fc.property(fc.array(arbBar, { minLength: 5, maxLength: 60 }), (bars) => {
                const out = harness(
                    bars,
                    bars.length + 1,
                    (bar) => tema("slot", bar.close, 4).current,
                );
