...
describe("ta.highestbars — property invariants", () => {
    ...
    it("the bar at the returned offset equals the rolling max value", () => {
        fc.assert(
            fc.property(fc.array(arbBar, { minLength: 20, maxLength: 80 }), (bars) => {
                ...
                for (let i = length - 1; i < bars.length; i += 1) {
                    ...
                    const offset = out[i];
                    expect(bars[i + offset].high).toBeCloseTo(expected, 10);
                }
            }),
            { numRuns: 30 },
        );
