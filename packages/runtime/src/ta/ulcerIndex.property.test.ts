...
describe("ta.ulcerIndex — property invariants", () => {
            ...
            fc.property(
                ...
                (bars, length) => {
                    const out = harness(
                        bars,
                        bars.length + 1,
                        (bar) => ulcerIndex("slot", bar.close, length).current,
                    );
    ...
    it("output is always non-negative (range invariant)", () => {
        fc.assert(
    ...
    it("strictly monotone-increasing source yields zero ulcer past warmup", () => {
        fc.assert(
            fc.property(
