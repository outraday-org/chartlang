...
import type { AnchorTriple, TriangleState } from "@invinite-org/chartlang-core";
...
describe("draw.triangle — script-facing throw", () => {
    it("throws the sentinel when called without a compiler-injected slot id", () => {
        expect(() => triangle(TRIPLE)).toThrow(
            "draw.triangle called outside an active script step",
        );
    });
    ...
    it("throws when slotId is provided but anchors are missing", () => {
        ...
        expect(() => triangle("slot", undefined as unknown as AnchorTriple)).toThrow(
            "draw.triangle called outside an active script step",
        );
    });

    it("throws when called inside the compiled overload path but outside an active context", () => {
        expect(() => triangle("slot", TRIPLE)).toThrow(
            "draw.triangle called outside an active script step",
        );
    });
});

describe("draw.triangle — happy path", () => {
    it("returns a DrawingHandle with stable id and emits op: create with the triangle state", () => {
        const { ctx, emissions } = makeCtx();
        ACTIVE_RUNTIME_CONTEXT.current = ctx;
        const handle = triangle("a.chart.ts:1:1#0", TRIPLE, {
            stroke: "#ef4444",
            fill: "#fee2e2",
            fillAlpha: 0.5,
        });
        ...
        expect(e.drawingKind).toBe("triangle");
        expect(e.op).toBe("create");
        const state = e.state as TriangleState;
        expect(state.kind).toBe("triangle");
        expect(state.anchors).toEqual(TRIPLE);
        expect(state.style.fillAlpha).toBe(0.5);
    });

    it("defaults opts to {} when omitted", () => {
        const { ctx, emissions } = makeCtx();
        ACTIVE_RUNTIME_CONTEXT.current = ctx;
        triangle("slot", TRIPLE);
        const state = emissions.drawings[0].state as TriangleState;
        expect(state.style).toEqual({});
    });
});

describe("draw.triangle — capability gating", () => {
    it("drops + diagnoses unsupported-drawing-kind when capabilities omit triangle", () => {
...
describe("draw.triangle — budget gating", () => {
