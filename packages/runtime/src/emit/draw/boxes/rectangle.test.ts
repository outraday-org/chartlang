...
import type { RectangleState, WorldPoint } from "@invinite-org/chartlang-core";
...
describe("draw.rectangle — script-facing throw", () => {
    it("throws the sentinel when called without a compiler-injected slot id", () => {
        expect(() => rectangle(A, B)).toThrow("draw.rectangle called outside an active script step");
    });

    it("throws when slotId is provided but the second anchor is missing", () => {
        const { ctx } = makeCtx();
        ACTIVE_RUNTIME_CONTEXT.current = ctx;
        expect(() => rectangle("slot", A, undefined as unknown as WorldPoint)).toThrow(
            "draw.rectangle called outside an active script step",
        );
    });

    it("throws when called inside the compiled overload path but outside an active context", () => {
        expect(() => rectangle("slot", A, B)).toThrow(
            "draw.rectangle called outside an active script step",
        );
    });
});

describe("draw.rectangle — happy path", () => {
    it("returns a DrawingHandle with stable id and emits op: create with the rectangle state", () => {
        const { ctx, emissions } = makeCtx();
        ACTIVE_RUNTIME_CONTEXT.current = ctx;
        const handle = rectangle("a.chart.ts:1:1#0", A, B, {
            stroke: "#3b82f6",
            fill: "#dbeafe",
            lineWidth: 2,
            fillAlpha: 0.4,
        ...
        expect(e.drawingKind).toBe("rectangle");
        expect(e.op).toBe("create");
        const state = e.state as RectangleState;
        expect(state.kind).toBe("rectangle");
        expect(state.anchors).toEqual([A, B]);
        expect(state.style.stroke).toBe("#3b82f6");
        expect(state.style.fill).toBe("#dbeafe");
        expect(state.style.fillAlpha).toBe(0.4);
    });
    ...
    it("defaults opts to {} when omitted", () => {
        ...
        ACTIVE_RUNTIME_CONTEXT.current = ctx;
        rectangle("slot", A, B);
        const state = emissions.drawings[0].state as RectangleState;
        expect(state.style).toEqual({});
    });
});

describe("draw.rectangle — capability gating", () => {
    it("drops + diagnoses unsupported-drawing-kind when capabilities.drawings lacks rectangle", () => {
...
describe("draw.rectangle — budget gating", () => {
