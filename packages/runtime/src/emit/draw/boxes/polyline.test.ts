...
import type { PolylineState, WorldPoint } from "@invinite-org/chartlang-core";
...
describe("draw.polyline — script-facing throw", () => {
    it("throws the sentinel when called without a compiler-injected slot id", () => {
        expect(() => polyline(POINTS)).toThrow(
            "draw.polyline called outside an active script step",
        );
    });
    ...
    it("throws when slotId is provided but anchors are missing", () => {
        ...
        expect(() =>
            polyline("slot", undefined as unknown as ReadonlyArray<WorldPoint>),
        ).toThrow("draw.polyline called outside an active script step");
    });

    it("throws when called inside the compiled overload path but outside an active context", () => {
        expect(() => polyline("slot", POINTS)).toThrow(
            "draw.polyline called outside an active script step",
        );
    });
});

describe("draw.polyline — happy path", () => {
    it("returns a DrawingHandle with stable id and emits op: create with the polyline state", () => {
        const { ctx, emissions } = makeCtx();
        ACTIVE_RUNTIME_CONTEXT.current = ctx;
        const handle = polyline("a.chart.ts:1:1#0", POINTS, { color: "#a855f7", lineWidth: 2 });
        expect(handle.id).toBe("a.chart.ts:1:1#0#0");
        expect(emissions.drawings).toHaveLength(1);
        const [e] = emissions.drawings;
        expect(e.drawingKind).toBe("polyline");
    ...
    it("defaults opts to {} when omitted", () => {
        const { ctx, emissions } = makeCtx();
        ACTIVE_RUNTIME_CONTEXT.current = ctx;
        polyline("slot", POINTS);
        const state = emissions.drawings[0].state as PolylineState;
        expect(state.style).toEqual({});
    });
});

describe("draw.polyline — capability gating", () => {
    it("drops + diagnoses unsupported-drawing-kind when capabilities omit polyline", () => {
...
describe("draw.polyline — budget gating", () => {
