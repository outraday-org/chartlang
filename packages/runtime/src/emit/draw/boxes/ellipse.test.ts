...
import type { EllipseState, WorldPoint } from "@invinite-org/chartlang-core";
...
const B: WorldPoint = { time: 1_700_000_120_000, price: 120 };

afterEach(() => {
    ACTIVE_RUNTIME_CONTEXT.current = null;
});

describe("draw.ellipse — script-facing throw", () => {
    it("throws the sentinel when called without a compiler-injected slot id", () => {
        expect(() => ellipse(A, B)).toThrow("draw.ellipse called outside an active script step");
    });

    it("throws when slotId is provided but the second anchor is missing", () => {
        const { ctx } = makeCtx();
        ACTIVE_RUNTIME_CONTEXT.current = ctx;
        expect(() => ellipse("slot", A, undefined as unknown as WorldPoint)).toThrow(
            "draw.ellipse called outside an active script step",
        );
    });

    it("throws when called inside the compiled overload path but outside an active context", () => {
        expect(() => ellipse("slot", A, B)).toThrow(
            "draw.ellipse called outside an active script step",
        );
    });
});

describe("draw.ellipse — happy path", () => {
    it("returns a DrawingHandle and emits op: create with the ellipse state", () => {
        const { ctx, emissions } = makeCtx();
        ACTIVE_RUNTIME_CONTEXT.current = ctx;
        const handle = ellipse("a.chart.ts:1:1#0", A, B, {
            stroke: "#22c55e",
            fill: "#dcfce7",
            fillAlpha: 0.3,
        });
        ...
        expect(e.drawingKind).toBe("ellipse");
        const state = e.state as EllipseState;
        expect(state.kind).toBe("ellipse");
        expect(state.anchors).toEqual([A, B]);
        expect(state.style.fill).toBe("#dcfce7");
    });
    ...
    it("defaults opts to {} when omitted", () => {
        ...
        ACTIVE_RUNTIME_CONTEXT.current = ctx;
        ellipse("slot", A, B);
        const state = emissions.drawings[0].state as EllipseState;
        expect(state.style).toEqual({});
    });
});

describe("draw.ellipse — capability gating", () => {
    it("drops + diagnoses unsupported-drawing-kind when capabilities omit ellipse", () => {
...
describe("draw.ellipse — budget gating", () => {
