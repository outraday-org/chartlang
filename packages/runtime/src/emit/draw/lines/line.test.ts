...
describe("draw.line — script-facing throw", () => {
    it("throws the sentinel when called without a compiler-injected slot id", () => {
        expect(() => line(A, B)).toThrow("draw.line called outside an active script step");
    });

    it("throws when slotId is provided but a/b are missing (defensive)", () => {
        const { ctx } = makeCtx();
        ACTIVE_RUNTIME_CONTEXT.current = ctx;
        expect(() => line("slot", undefined as unknown as WorldPoint, B)).toThrow(
            "draw.line called outside an active script step",
        );
    });

    it("throws when called inside the compiled overload path but outside an active context", () => {
        expect(() => line("slot", A, B)).toThrow("draw.line called outside an active script step");
    });
});

describe("draw.line — happy path", () => {
    it("returns a DrawingHandle with stable id and emits op: create with the line state", () => {
        const { ctx, emissions } = makeCtx();
        ACTIVE_RUNTIME_CONTEXT.current = ctx;
        const handle = line("a.chart.ts:1:1#0", A, B, { color: "#3b82f6", lineWidth: 2 });
    ...
    it("defaults opts to {} when omitted", () => {
        ...
        ACTIVE_RUNTIME_CONTEXT.current = ctx;
        line("slot", A, B);
        const state = emissions.drawings[0].state as LineState;
        expect(state.style).toEqual({});
    });
});

describe("draw.line — capability gating", () => {
    it("drops + diagnoses unsupported-drawing-kind when capabilities.drawings lacks line", () => {
...
describe("draw.line — budget gating", () => {
