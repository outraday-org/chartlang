...
import type { HorizontalRayState, WorldPoint } from "@invinite-org/chartlang-core";
...
import { horizontalRay } from "./horizontalRay.js";
...
describe("draw.horizontalRay — script-facing throw", () => {
    it("throws the sentinel when called without a compiler-injected slot id", () => {
        expect(() => horizontalRay(ANCHOR)).toThrow(
            "draw.horizontalRay called outside an active script step",
        );
    });
    ...
    it("throws when slotId is provided but anchor is missing", () => {
        ...
        expect(() => horizontalRay("slot", undefined as unknown as WorldPoint)).toThrow(
            "draw.horizontalRay called outside an active script step",
        );
    });

    it("throws when called inside the compiled overload path but outside an active context", () => {
        expect(() => horizontalRay("slot", ANCHOR)).toThrow(
            "draw.horizontalRay called outside an active script step",
        );
    });
});

describe("draw.horizontalRay — happy path + gating", () => {
    it("emits op: create with the horizontal-ray state and a stable handle id", () => {
        const { ctx, emissions } = makeCtx();
        ACTIVE_RUNTIME_CONTEXT.current = ctx;
        const handle = horizontalRay("a.chart.ts:1:1#0", ANCHOR, { color: "#10b981" });
        expect(handle.id).toBe("a.chart.ts:1:1#0#0");
        const state = emissions.drawings[0].state as HorizontalRayState;
        expect(state.kind).toBe("horizontal-ray");
    ...
    it("defaults opts to {} when omitted", () => {
        const { ctx, emissions } = makeCtx();
        ACTIVE_RUNTIME_CONTEXT.current = ctx;
        horizontalRay("slot", ANCHOR);
        expect((emissions.drawings[0].state as HorizontalRayState).style).toEqual({});
    });

    it("drops + diagnoses unsupported-drawing-kind when capabilities omit horizontal-ray", () => {
