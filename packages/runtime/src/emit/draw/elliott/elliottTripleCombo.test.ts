...
import type { AnchorHept, ElliottTripleComboState } from "@invinite-org/chartlang-core";
...
import { elliottTripleCombo } from "./elliottTripleCombo.js";

function makeCaps(overrides: Partial<Capabilities> = {}): Capabilities {
    return {
        plots: new Set(),
        drawings: new Set(["elliott-triple-combo"]),
        alerts: new Set(),
...
describe("draw.elliottTripleCombo — script-facing throw", () => {
    it("throws the sentinel when called without a compiler-injected slot id", () => {
        expect(() => elliottTripleCombo(ANCHORS)).toThrow(
            "draw.elliottTripleCombo called outside an active script step",
        );
    });
    ...
    it("throws when slotId is provided but anchors are missing", () => {
        ...
        expect(() => elliottTripleCombo("slot", undefined as unknown as AnchorHept)).toThrow(
            "draw.elliottTripleCombo called outside an active script step",
        );
    });

    it("throws when invoked through the compiled overload outside an active context", () => {
        expect(() => elliottTripleCombo("slot", ANCHORS)).toThrow(
            "draw.elliottTripleCombo called outside an active script step",
        );
    });
});

describe("draw.elliottTripleCombo — happy path", () => {
    it("returns a DrawingHandle and emits op:create with the elliott-triple-combo state", () => {
        const { ctx, emissions } = makeCtx();
        ACTIVE_RUNTIME_CONTEXT.current = ctx;
        const handle = elliottTripleCombo("etc.chart.ts:1:1#0", ANCHORS, {
            color: "#14b8a6",
        });
        expect(handle.id).toBe("etc.chart.ts:1:1#0#0");
        const state = emissions.drawings[0].state as ElliottTripleComboState;
        expect(state.kind).toBe("elliott-triple-combo");
    ...
    it("defaults opts to {} when omitted", () => {
        const { ctx, emissions } = makeCtx();
        ACTIVE_RUNTIME_CONTEXT.current = ctx;
        elliottTripleCombo("slot", ANCHORS);
        const state = emissions.drawings[0].state as ElliottTripleComboState;
    ...
    it("forwards opts.labels onto state.labels and strips it from style", () => {
        ...
        ACTIVE_RUNTIME_CONTEXT.current = ctx;
        elliottTripleCombo("slot", ANCHORS, {
            labels: ["S", "W", "X1", "Y", "X2", "Zi", "Z"],
        } as unknown as Parameters<typeof elliottTripleCombo>[2]);
        const state = emissions.drawings[0].state as ElliottTripleComboState;
        expect(state.labels).toEqual(["S", "W", "X1", "Y", "X2", "Zi", "Z"]);
        expect(state.style).toEqual({});
    });
});

describe("draw.elliottTripleCombo — capability gating", () => {
    it("drops + diagnoses unsupported-drawing-kind when caps omit elliott-triple-combo", () => {
...
describe("draw.elliottTripleCombo — budget gating", () => {
