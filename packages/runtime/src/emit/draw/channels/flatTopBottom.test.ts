...
import type { AnchorTriple, FlatTopBottomState } from "@invinite-org/chartlang-core";
...
import { flatTopBottom } from "./flatTopBottom.js";
...
describe("draw.flatTopBottom — script-facing throw", () => {
    it("throws the sentinel when called without a compiler-injected slot id", () => {
        expect(() => flatTopBottom(ANCHORS)).toThrow(
            "draw.flatTopBottom called outside an active script step",
        );
    });
    ...
    it("throws when slotId is provided but anchors are missing", () => {
        ...
        expect(() => flatTopBottom("slot", undefined as unknown as AnchorTriple)).toThrow(
            "draw.flatTopBottom called outside an active script step",
        );
    });

    it("throws when called inside the compiled overload path but outside an active context", () => {
        expect(() => flatTopBottom("slot", ANCHORS)).toThrow(
            "draw.flatTopBottom called outside an active script step",
        );
    });
});

describe("draw.flatTopBottom — happy path", () => {
    it("returns a DrawingHandle and emits op: create with the flat-top-bottom state", () => {
        const { ctx, emissions } = makeCtx();
        ACTIVE_RUNTIME_CONTEXT.current = ctx;
        const handle = flatTopBottom("ftb.chart.ts:1:1#0", ANCHORS, {
            color: "#3b82f6",
            lineStyle: "dashed",
        });
        expect(handle.id).toBe("ftb.chart.ts:1:1#0#0");
        expect(emissions.drawings).toHaveLength(1);
        const [e] = emissions.drawings;
        expect(e.drawingKind).toBe("flat-top-bottom");
        expect(e.op).toBe("create");
        const state = e.state as FlatTopBottomState;
        expect(state.kind).toBe("flat-top-bottom");
    ...
    it("defaults opts to {} when omitted", () => {
        const { ctx, emissions } = makeCtx();
        ACTIVE_RUNTIME_CONTEXT.current = ctx;
        flatTopBottom("slot", ANCHORS);
        const state = emissions.drawings[0].state as FlatTopBottomState;
        expect(state.style).toEqual({});
    });
});

describe("draw.flatTopBottom — capability gating", () => {
    it("drops + diagnoses unsupported-drawing-kind when capabilities omit flat-top-bottom", () => {
...
describe("draw.flatTopBottom — budget gating", () => {
