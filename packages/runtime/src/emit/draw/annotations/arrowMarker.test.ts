...
import type { ArrowMarkerState, WorldPoint } from "@invinite-org/chartlang-core";
...
import { arrowMarker } from "./arrowMarker.js";
...
describe("draw.arrowMarker — script-facing throw", () => {
    it("throws the sentinel when called without a compiler-injected slot id", () => {
        expect(() => arrowMarker(ANCHOR)).toThrow(
            "draw.arrowMarker called outside an active script step",
        );
    });
    ...
    it("throws when slotId is provided but anchor is missing", () => {
        ...
        expect(() => arrowMarker("slot", undefined as unknown as WorldPoint)).toThrow(
            "draw.arrowMarker called outside an active script step",
        );
    });

    it("throws when called inside the compiled overload path but outside an active context", () => {
        expect(() => arrowMarker("slot", ANCHOR)).toThrow(
            "draw.arrowMarker called outside an active script step",
        );
    });
});

describe("draw.arrowMarker — happy path", () => {
    it("returns a DrawingHandle and emits op: create with the arrow-marker state", () => {
        const { ctx, emissions } = makeCtx();
        ACTIVE_RUNTIME_CONTEXT.current = ctx;
        const handle = arrowMarker("a.chart.ts:1:1#0", ANCHOR, {
            color: "#10b981",
            text: "Long",
        });
        expect(handle.id).toBe("a.chart.ts:1:1#0#0");
        const state = emissions.drawings[0].state as ArrowMarkerState;
        expect(state.kind).toBe("arrow-marker");
        expect(state.anchor).toEqual(ANCHOR);
        expect(state.style.color).toBe("#10b981");
        expect(state.style.text).toBe("Long");
    });

    it("defaults opts to {} when omitted", () => {
        const { ctx, emissions } = makeCtx();
        ACTIVE_RUNTIME_CONTEXT.current = ctx;
        arrowMarker("slot", ANCHOR);
        const state = emissions.drawings[0].state as ArrowMarkerState;
        expect(state.style).toEqual({});
    });
});

describe("draw.arrowMarker — capability gating", () => {
    it("drops + diagnoses unsupported-drawing-kind when capabilities omit arrow-marker", () => {
...
describe("draw.arrowMarker — budget gating", () => {
