...
import type { ArrowMarkUpState, WorldPoint } from "@invinite-org/chartlang-core";
...
import { arrowMarkUp } from "./arrowMarkUp.js";
...
describe("draw.arrowMarkUp — script-facing throw", () => {
    it("throws the sentinel when called without a compiler-injected slot id", () => {
        expect(() => arrowMarkUp(ANCHOR)).toThrow(
            "draw.arrowMarkUp called outside an active script step",
        );
    });
    ...
    it("throws when slotId is provided but anchor is missing", () => {
        ...
        expect(() => arrowMarkUp("slot", undefined as unknown as WorldPoint)).toThrow(
            "draw.arrowMarkUp called outside an active script step",
        );
    });

    it("throws when called inside the compiled overload path but outside an active context", () => {
        expect(() => arrowMarkUp("slot", ANCHOR)).toThrow(
            "draw.arrowMarkUp called outside an active script step",
        );
    });
});

describe("draw.arrowMarkUp — happy path", () => {
    it("returns a DrawingHandle and emits op: create with the arrow-mark-up state", () => {
        const { ctx, emissions } = makeCtx();
        ACTIVE_RUNTIME_CONTEXT.current = ctx;
        const handle = arrowMarkUp("a.chart.ts:1:1#0", ANCHOR, { color: "#1e40af" });
        expect(handle.id).toBe("a.chart.ts:1:1#0#0");
        const state = emissions.drawings[0].state as ArrowMarkUpState;
        expect(state.kind).toBe("arrow-mark-up");
        expect(state.anchor).toEqual(ANCHOR);
        expect(state.style.color).toBe("#1e40af");
    });

    it("defaults opts to {} when omitted", () => {
        const { ctx, emissions } = makeCtx();
        ACTIVE_RUNTIME_CONTEXT.current = ctx;
        arrowMarkUp("slot", ANCHOR);
        const state = emissions.drawings[0].state as ArrowMarkUpState;
        expect(state.style).toEqual({});
    });
});

describe("draw.arrowMarkUp — capability gating", () => {
    it("drops + diagnoses unsupported-drawing-kind when capabilities omit arrow-mark-up", () => {
...
describe("draw.arrowMarkUp — budget gating", () => {
