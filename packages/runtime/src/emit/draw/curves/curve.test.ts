...
import type { AnchorTriple, CurveState } from "@invinite-org/chartlang-core";
...
describe("draw.curve — script-facing throw", () => {
    ...
    it("throws when slotId is provided but anchors are missing", () => {
        ...
        expect(() => curve("slot", undefined as unknown as AnchorTriple)).toThrow(
            "draw.curve called outside an active script step",
        );
    });

    it("throws when called inside the compiled overload path but outside an active context", () => {
        expect(() => curve("slot", ANCHORS)).toThrow(
            "draw.curve called outside an active script step",
        );
    });
});

describe("draw.curve — happy path", () => {
    it("returns a DrawingHandle and emits op: create with the curve state", () => {
        const { ctx, emissions } = makeCtx();
        ACTIVE_RUNTIME_CONTEXT.current = ctx;
        const handle = curve("a.chart.ts:1:1#0", ANCHORS, { color: "#22c55e" });
        expect(handle.id).toBe("a.chart.ts:1:1#0#0");
        const state = emissions.drawings[0].state as CurveState;
        expect(state.kind).toBe("curve");
...
describe("draw.curve — capability gating", () => {
    it("drops + diagnoses unsupported-drawing-kind when capabilities omit curve", () => {
...
describe("draw.curve — budget gating", () => {
