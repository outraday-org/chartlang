...
import type { FibSpeedArcsState, WorldPoint } from "@invinite-org/chartlang-core";
...
import { fibSpeedArcs } from "./fibSpeedArcs.js";

function makeCaps(overrides: Partial<Capabilities> = {}): Capabilities {
    return {
        plots: new Set(),
        drawings: new Set(["fib-speed-arcs"]),
        alerts: new Set(),
...
describe("draw.fibSpeedArcs — script-facing throw", () => {
    it("throws the sentinel when called without a compiler-injected slot id", () => {
        expect(() => fibSpeedArcs(A, B)).toThrow(
            "draw.fibSpeedArcs called outside an active script step",
        );
    });

    it("throws when invoked through the compiled overload outside an active context", () => {
        expect(() => fibSpeedArcs("slot", A, B)).toThrow(
            "draw.fibSpeedArcs called outside an active script step",
        );
    });
    ...
    it("throws when a/b is missing in the compiler-form", () => {
        ...
        ).toThrow("draw.fibSpeedArcs called outside an active script step");
    });
});

describe("draw.fibSpeedArcs — happy path", () => {
    it("returns a DrawingHandle and emits op: create with the fib-speed-arcs state", () => {
        const { ctx, emissions } = makeCtx();
        ACTIVE_RUNTIME_CONTEXT.current = ctx;
        const handle = fibSpeedArcs("arcs.chart.ts:1:1#0", A, B);
        expect(handle.id).toBe("arcs.chart.ts:1:1#0#0");
        const state = emissions.drawings[0].state as FibSpeedArcsState;
        expect(state.kind).toBe("fib-speed-arcs");
        expect(state.anchors).toEqual([A, B]);
        expect(state.style).toEqual({});
    });
});

describe("draw.fibSpeedArcs — capability gating", () => {
    it("drops + diagnoses unsupported-drawing-kind when capabilities omit fib-speed-arcs", () => {
        const caps = makeCaps({ drawings: new Set() });
        const { ctx, emissions } = makeCtx(caps);
        ACTIVE_RUNTIME_CONTEXT.current = ctx;
        fibSpeedArcs("slot", A, B);
        expect(emissions.drawings).toEqual([]);
        expect(emissions.diagnostics[0].code).toBe("unsupported-drawing-kind");
    });
});

describe("draw.fibSpeedArcs — budget gating", () => {
