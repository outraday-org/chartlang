...
import type { TimeCyclesState, WorldPoint } from "@invinite-org/chartlang-core";
...
import { timeCycles } from "./timeCycles.js";

function makeCaps(overrides: Partial<Capabilities> = {}): Capabilities {
    return {
        plots: new Set(),
        drawings: new Set(["time-cycles"]),
        alerts: new Set(),
...
describe("draw.timeCycles — script-facing throw", () => {
    it("throws the sentinel when called without a compiler-injected slot id", () => {
        expect(() => timeCycles(A, B)).toThrow(
            "draw.timeCycles called outside an active script step",
        );
    });
    ...
    it("throws when slotId is provided but anchors are missing", () => {
        ...
        expect(() => timeCycles("slot", undefined as unknown as WorldPoint, B)).toThrow(
            "draw.timeCycles called outside an active script step",
        );
    });

    it("throws when invoked through the compiled overload outside an active context", () => {
        expect(() => timeCycles("slot", A, B)).toThrow(
            "draw.timeCycles called outside an active script step",
        );
    });
});

describe("draw.timeCycles — happy path", () => {
    it("returns a DrawingHandle and emits op:create with the time-cycles state", () => {
        const { ctx, emissions } = makeCtx();
        ACTIVE_RUNTIME_CONTEXT.current = ctx;
        const handle = timeCycles("t.chart.ts:1:1#0", A, B, { color: "#0ea5e9" });
        expect(handle.id).toBe("t.chart.ts:1:1#0#0");
        const state = emissions.drawings[0].state as TimeCyclesState;
        expect(state.kind).toBe("time-cycles");
...
describe("draw.timeCycles — capability gating", () => {
    it("drops + diagnoses unsupported-drawing-kind when caps omit time-cycles", () => {
...
describe("draw.timeCycles — budget gating", () => {
