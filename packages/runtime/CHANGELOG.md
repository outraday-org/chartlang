...
- 3932418: Emit the `orders` channel, fold a nominal position, and auto-render its markers
  ...
  halt and a dep error discard the bar's orders _and_ its pending fold together, so
  ...
  **Two changes to surfaces that predate this feature, worth reading even if you
  ...
  - **`AlertEmission.meta` is now DEEP-FROZEN.** There were two divergent private
    ...
    helper (`emit/snapshotMeta.ts`) that clones _and_ freezes every level, shared
...
- 1039309: Fix two external-series feed bugs that made a bound `input.externalSeries`
  read render nothing.
