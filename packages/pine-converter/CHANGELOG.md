...
- 65491dc: Never emit a misplaced `strategy.long` / `strategy.short` into a lowered `qty:`
...
- 3932418: Lower `strategy.*` to `order.*` market intents instead of `alert(...)`
...
- 166222f: Split the fs-touching API out of the pine-converter root entry so the package
