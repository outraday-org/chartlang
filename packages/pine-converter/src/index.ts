 ...
 * Reading and writing files is the `convertFile` wrapper's job and lives on
 * the `@invinite-org/chartlang-pine-converter/node` sub-export — this entry
 * deliberately imports no Node built-in at any depth of its module graph, so
 * it resolves in browsers, Deno and workers.
 * Lex/parse error-severity diagnostics short-circuit with a `null` output.
 *
 * @since 0.1
 * @stable
 * @example
