 ...
 * caller issues `moveTo(pts[0])` then one `bezierCurveTo` per segment (a
 * canvas2d adapter), or samples each segment into denser points (the GPU
 * line-strip adapter, which has no native curve). Fewer than two points
 * yields `[]`. A zero/negative-width gap (coincident x) degrades that one
 * segment to a straight line, so the path never divides by zero.
 *
 * Shared by the canvas2d reference adapter (direct `bezierCurveTo`) and the
 * webgl example adapter (samples the curve into line-strip points) — the
 * single monotone-cubic source so the five smoothing adapters stay in
 * visual parity. Do not fork it per adapter.
 *
 * @since 1.7
 * @stable
 * @example
 ...
 *     void segs;
 */
export function monotoneCubicSegments(pts: ReadonlyArray<Point2>): BezierSegment[] {
    const n = pts.length;
    if (n < 2) return [];
